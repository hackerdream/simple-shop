<template>
  <div class="checkout cart-page">
    <nav-header v-on:loginChange="reGetCartList"></nav-header>
    <nav-bread :itemName="'cart'"></nav-bread>
              ...
              <li v-for="(item,index) of cartList" :key="item.productId">
                  ...
                  <div class="item-quantity">
                    <div class="select-self select-self-open">
                      <div class="select-self-area">
                        <a class="input-sub" @click="editNum('sub',item)">-</a>
                        <span class="select-ipt" @dblclick="editNumByInp(item.productNum,index)">{{item.productNum}}</span>
                        <input :class="{'noshow':isEditNum !== index}" @keyup.enter="changeNum(item)" v-model="inputNum" type="number" style="width:40px;height: 28px;position: absolute;left: 41px;top: 0px;padding:5px">
                ...
                <div class="cart-tab-5">
                  <div class="cart-item-opration">
                    <a href="javascript:;" class="item-edit-btn" @click="deleteGood(item.productId)">
          ...
          <div class="cart-foot-inner">
            <div class="cart-foot-l">
              <div class="item-all-check">
                <a href="javascipt:;">
                  <span class="checkbox-btn item-check-btn" v-bind:class="{'check':isSelectAll==true}" @click="toggleSellectAll">
                    <svg class="icon icon-ok">
    ...
    methods: {
      ...
      editCart(item) {
        ...
        this.cartList.forEach(function (good) {
          ...
          if (good.productId === item.productId) {
            good.checked = item.checked === '1' ? '0' : '1'
            that.$http.post("/users/editCart", {
      ...
      deleteGood(id) {
        this.$http.delete('/users/deleteGood?productId=' + id).then((res) => {
          if (res.data.status === 1) {
            console.log("网络不好删除失败");
          } else {
            var that = this;
      ...
      editNum(action, item) {
        ...
        if (action === 'sub') {
          if (num > 1) {
            item.productNum = num - 1;
          }
        } else if (action === 'add') {
          item.productNum = num + 1;
        }
        this.$http.post("/users/editCart", {
      ...
      editNumByInp(num, index) {
        this.inputNum = num;
        this.isEditNum = index;
      },
      toggleSellectAll() {
        var select = this.isSelectAll === true ? false : true;

        this.$http.post('/users/selectAll', {
          selectAll: select
        }).then((res) => {
          if (res.data.status === 0) {
            this.getCartList();
